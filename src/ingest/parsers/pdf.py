...
def parse_pdf(path: Path) -> ParsedDocument:
    ...
    for index, page in enumerate(document, start=1):
        ...
        sections.append(ParsedSection(text=text.strip(), page_number=index, breadcrumbs=breadcrumbs))
    document.close()
...
def _extract_tables(path: Path) -> Iterable[ParsedSection]:  # noqa: PLR0912
    ...
    if camelot is not None:  # pragma: no cover - requires camelot dependencies
        ...
        for index, table in enumerate(tables, start=1):
            ...
            rows = cleaned.iloc[1:]
            lines = [" | ".join(str(cell).strip() for cell in row.tolist()) for _, row in rows.iterrows()]
            content = "\n".join(lines)
