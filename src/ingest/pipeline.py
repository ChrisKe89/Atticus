...
def ingest_corpus(  # noqa: PLR0915, PLR0912
        ...
        if manifest_entry and manifest_entry.get("sha256") == file_hash:
            ...
            if reused:
                for item in reused:
                    ...
                    item.extra.setdefault("embedding_model_version", settings.embedding_model_version)
    ...
    for index_counter, (parsed_chunk, embedding) in enumerate(
        ...
        metadata.setdefault("token_count", str(max(0, parsed_chunk.end_token - parsed_chunk.start_token)))
