...
async def ask(payload: AskPayload, request: Request) -> dict[str, Any]:
    ...
    result = answer_question(q, settings=settings, logger=logger, request_id=request_id)
    ...
    response_payload = {
        ...
        "bullets": result.bullets or None,
        "sources": sources,
    ...
    if getattr(settings, "verbose_logging", False):
        try:
            ...
            user_tokens = None
        try:
            answer_tokens = count_tokens(result.summary or "")
        except Exception:
            answer_tokens = None
        decision_trace: list[str] = []
        if getattr(settings, "trace_logging", False):
            decision_trace.append("received_question")
            decision_trace.append(f"retrieved_citations={len(getattr(result, 'citations', []) or [])}")
        ...
        request_id = request_id or getattr(request.state, "request_id", "unknown")
        log_event(
            logger,
            "chat_turn",
