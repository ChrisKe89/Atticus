...
class VectorStore:
    ...
    def _rerank_results(self, results: list[SearchResult]) -> list[SearchResult]:
        ...
        reranked = sorted(
            results,
            key=lambda item: (
                vector_weight * item.vector_score + lexical_weight * item.lexical_score + fuzz_weight * item.fuzz_score
            ),
            reverse=True,
        )
    ...
    def search(
        ...
        top_lex = sorted(range(len(bm25_all)), key=lambda i: bm25_all[i], reverse=True)[: max(top_k * 3, 30)]
        for i in top_lex:
