...
class GeneratorClient:
        ...
        if any(key in lowered_prompt for key in ["resolution", "dpi", "print resolution"]):
            ...
            for block in contexts:
                ...
                for line in block.splitlines():
                    m_iter = re.finditer(
                        rf"\b(?:up to\s*)?(?:\d{{2,4}}\s*[x{MULTIPLICATION_SIGN}]\s*\d{{2,4}}|\d{{2,4}})\s*dpi\b(?:\s*[A-Za-z/ ]*)?",
                        line,
                        flags=re.IGNORECASE,
                    )
                    for m in m_iter:
    ...
    def heuristic_confidence(self, answer: str) -> float:
        ...
        if "dpi" in lowered:
            ...
            if re.search(
                rf"\b\d{{2,4}}\s*(?:[x{MULTIPLICATION_SIGN}]\s*\d{{2,4}}\s*)?dpi\b",
                lowered,
            ):
                return 0.9
        ...
        if "confidence" in lowered and "%" in lowered:
            try:
                percent_tokens = (int(token.strip("%")) for token in lowered.split() if token.strip("% ").isdigit())
