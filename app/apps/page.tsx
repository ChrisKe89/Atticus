...
const integrations = [
  {
    ...
    description: 'Sync product sheets, tables, and field notes from Postgres with deterministic chunking.',
    icon: Layers,
    href: '#prisma',
  },
  {
    name: 'Salesforce service cloud',
    description: 'Escalate chat transcripts to a case queue with the full retrieval trace and request ID.',
    icon: KanbanSquare,
    href: '#salesforce',
  },
  {
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-10">
      ...
      <section className="grid gap-6 md:grid-cols-2">
        {integrations.map((integration) => (
          <article key={integration.name} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{integration.name}</h2>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{integration.description}</p>
            <Link
