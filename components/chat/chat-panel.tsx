  ...
  return (
    <section className="grid gap-10 lg:grid-cols-[1.3fr_1fr]">
      <div className="flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              >
                    ...
                    <ul className="space-y-1 text-xs text-slate-600 dark:text-slate-300">
                      {message.response.sources.map((source: AskSource, index) => (
                        <li key={`${message.id}-source-${index}`} className="flex items-start gap-2">
                          <span className="mt-0.5 inline-flex h-2 w-2 flex-none rounded-full bg-indigo-500" aria-hidden="true" />
                          <span>
                            {source.path}
                ...
                {message.response ? (
                  <footer className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <span>
                      Escalate: {message.response.should_escalate ? (
                ...
                {message.status === 'error' ? (
                  <p className="mt-2 text-xs text-rose-600">{error ?? 'Try asking again in a few minutes.'}</p>
                ) : null}
              </div>
            </article>
          ))}
        ...
        <form onSubmit={handleSubmit} className="mt-6 rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            {error ? <span className="text-rose-600">{error}</span> : lastAssistant ? <span className="truncate text-slate-400">Last request: {lastAssistant.request_id}</span> : null}
          </div>
        </form>
      </div>
