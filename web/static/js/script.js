...
function loadChat(chatOutput) {
    ...
    if (history) chatOutput.innerHTML = history;
}
...
/* ---------- Sidebar collapse logic (icon button) ---------- */
...
function setSidebarState(collapsed) {
    ...
    // - Keep icon at ~40px from left in both states (icon-col is 80px wide).
    document.querySelectorAll(".icon-col").forEach((el) => {
        el.classList.toggle("icon-col--collapsed", collapsed);
        el.classList.toggle("icon-col--expanded", !collapsed);
    });

    // Set aria state on all toggles
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Hook up the toggle button(s)
    document.querySelectorAll("[data-sidebar-toggle]").forEach((btn) => {
        btn.addEventListener("click", toggleSidebar);
    });

    // Initialize sidebar state from current classes (supports SSR/defaults)
    (function initSidebar() {
        const sidebar = document.getElementById("sidebar");
        if (!sidebar) return;
        const collapsed = sidebar.classList.contains("sidebar-collapsed") || sidebar.classList.contains("w-20");
        setSidebarState(collapsed);
    })();

    // ESC to collapse
    document.addEventListener("keydown", (evt) => {
        if (evt.key === "Escape") setSidebarState(true);
    });

    // ----- Chat wiring (existing behavior) -----
    const chatForm = document.getElementById("chat-form");
    const chatInput = document.getElementById("chat-input");
    const chatOutput = document.getElementById("chat-stream") ?? document.getElementById("chat-output");
    ...
    if (chatForm && chatInput && chatOutput) {
        ...
        function renderBotReply({ answer, sources, confidence, escalated, ae_id, request_id }) {
            ...
            const metaChunks = [];
            if (typeof confidence === "number") metaChunks.push(`Confidence: ${(confidence * 100).toFixed(0)}%`);
            if (request_id) metaChunks.push(`Request ID: ${request_id}`);
            const metaText = metaChunks.length ? metaChunks.join(" · ") : undefined;
            addMessage(parts.join("\n\n"), "bot", { sources, metaText });
